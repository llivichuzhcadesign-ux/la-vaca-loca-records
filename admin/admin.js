...
function mediaField(path,label,value,hint=''){
  ...
  const preview=value?`<div class="media-preview has-image"><img src="${safe}" alt="${escapeText(label)} preview" onerror="this.parentElement.classList.add('broken')"></div>`:`<div class="media-preview"><span>No image yet</span></div>`;
  return `<div class="field media-field full"><label>${label}</label><div class="media-row">${preview}<div class="media-controls"><input type="text" value="${safe}" data-path="${path}" placeholder="assets/images/example.jpg or https://..."><small>${hint||'Paste a local asset path or external image URL. Real upload comes in the next storage phase.'}</small><button class="admin-action secondary" type="button" data-copy-path="${path}">Copy path</button></div></div></div>`;
}
...
function renderRecords(){
  ...
  byId('recordsList').innerHTML=list.length?list.map((r,i)=>card('records',i,r.artist||'Untitled record',`${r.id||'NO ID'} / ${r.genre||'Genre'}`,r.status,[
    field(`records.${i}.id`,'ID',r.id),field(`records.${i}.artist`,'Artist',r.artist),field(`records.${i}.title`,'Title',r.title),field(`records.${i}.genre`,'Genre',r.genre),field(`records.${i}.price`,'Price',r.price,'number'),field(`records.${i}.stock`,'Stock',r.stock,'number'),selectField(`records.${i}.status`,'Status',r.status,STATUS_OPTIONS.records),toggleField(`records.${i}.featured`,'Featured',!!r.featured,'Use for homepage or highlighted store sections later.'),toggleField(`records.${i}.featuredInPlayer`,'Featured in player',!!r.featuredInPlayer,'Can be used for random/player recommendations.'),toggleField(`records.${i}.hideFromPublic`,'Hide from public',!!r.hideFromPublic,'Keep the item in admin without showing it publicly.'),field(`records.${i}.condition`,'Condition',r.condition),field(`records.${i}.label`,'Label',r.label),field(`records.${i}.year`,'Year',r.year),mediaField(`records.${i}.coverImage`,'Cover image',r.coverImage,'Square record cover image for product cards.'),field(`records.${i}.audioPreview`,'Audio preview path',r.audioPreview),field(`records.${i}.description`,'Description',r.description,'textarea')
...
function renderSessions(){
  ...
  byId('sessionsList').innerHTML=list.length?list.map((s,i)=>card('sessions',i,s.title||'Untitled session',`${s.id||'NO ID'} / ${s.type||'Type'}`,s.status,[
    field(`sessions.${i}.id`,'ID',s.id),field(`sessions.${i}.title`,'Title',s.title),field(`sessions.${i}.type`,'Type',s.type),field(`sessions.${i}.date`,'Date',s.date),selectField(`sessions.${i}.status`,'Status',s.status,STATUS_OPTIONS.sessions),toggleField(`sessions.${i}.featured`,'Featured',!!s.featured),toggleField(`sessions.${i}.hideFromPublic`,'Hide from public',!!s.hideFromPublic),mediaField(`sessions.${i}.heroImage`,'Session hero image',s.heroImage,'Wide image for Sessions cards and archive links.'),field(`sessions.${i}.audio`,'Audio path',s.audio),field(`sessions.${i}.video`,'Video path',s.video),field(`sessions.${i}.relatedRecords`,'Related records',Array.isArray(s.relatedRecords)?s.relatedRecords.join(', '):s.relatedRecords,'text','Comma-separated record IDs.'),field(`sessions.${i}.detail`,'Description',s.detail,'textarea')
  ].join(''),s)).join(''):'<div class="empty">No sessions yet.</div>';
}
function renderEvents(){
  const list=draft.events||[];
  byId('eventsList').innerHTML=list.length?list.map((ev,i)=>card('events',i,ev.title||'Untitled event',`${ev.date||'Date'} / ${ev.place||'Place'}`,ev.status,[
    field(`events.${i}.id`,'ID',ev.id),field(`events.${i}.title`,'Title',ev.title),field(`events.${i}.date`,'Date',ev.date),field(`events.${i}.place`,'Place',ev.place),selectField(`events.${i}.status`,'Status',ev.status,STATUS_OPTIONS.events),toggleField(`events.${i}.featured`,'Featured',!!ev.featured),toggleField(`events.${i}.hideFromPublic`,'Hide from public',!!ev.hideFromPublic),mediaField(`events.${i}.posterImage`,'Poster image',ev.posterImage,'Poster/flyer image for event cards.'),field(`events.${i}.detail`,'Description',ev.detail,'textarea')
  ].join(''),ev)).join(''):'<div class="empty">No events yet.</div>';
}
function renderArchive(){
  ...
  byId('archiveList').innerHTML=list.length?list.map((a,i)=>card('archiveItems',i,a.title||'Untitled archive item',`${a.id||'NO ID'} / ${a.category||'Category'}`,a.relatedSession||a.relatedEvent||'archive',[
    field(`archiveItems.${i}.id`,'ID',a.id),field(`archiveItems.${i}.title`,'Title',a.title),field(`archiveItems.${i}.category`,'Category',a.category),toggleField(`archiveItems.${i}.featured`,'Featured',!!a.featured),toggleField(`archiveItems.${i}.hideFromPublic`,'Hide from public',!!a.hideFromPublic),mediaField(`archiveItems.${i}.image`,'Archive image',a.image,'Image for archive, space, poster, or culture entry.'),field(`archiveItems.${i}.relatedSession`,'Related session',a.relatedSession),field(`archiveItems.${i}.relatedEvent`,'Related event',a.relatedEvent),field(`archiveItems.${i}.detail`,'Description',a.detail,'textarea')
...
function addItem(type){
  ...
  draft[type]=draft[type]||[];draft[type].push(templates[type]);saveDraft();
}
