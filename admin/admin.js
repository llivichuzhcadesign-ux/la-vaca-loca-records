...
async function handleHeroUpload(input){
  ...
  try{
    ...
    markDirty();saveDraft();showToast('Original image saved unchanged');
...
function compressImage(file,maxSize=1600,quality=.82){return new Promise((resolve,reject)=>{if(!file||!file.type.startsWith('image/')){reject(new Error('Please choose an image file'));return}const reader=new FileReader();reader.onerror=()=>reject(new Error('Could not read image'));reader.onload=()=>{const img=new Image();img.onerror=()=>reject(new Error('Could not load image'));img.onload=()=>{let {width,height}=img;const scale=Math.min(1,maxSize/Math.max(width,height));width=Math.round(width*scale);height=Math.round(height*scale);const canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;const ctx=canvas.getContext('2d');ctx.drawImage(img,0,0,width,height);resolve(canvas.toDataURL('image/jpeg',quality))};img.src=reader.result};reader.readAsDataURL(file)})}
async function handleImageUpload(input){const file=input.files&&input.files[0];if(!file)return;try{showToast('Preparing image...');const dataUrl=await compressImage(file);setValue(input.dataset.uploadPath,dataUrl);if(saveDraft())showToast('Image ready for draft preview')}catch(error){showToast(error.message||'Image upload failed')}}
